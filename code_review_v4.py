...
# Função para buscar todos os pull requests de um repositório
def obter_pull_requests(organization, project, repo, pat):
    url = f"https://dev.azure.com/{organization}/{project}/_apis/git/repositories/{repo}/pullRequests?api-version=6.0"
    headers = {
    ...
    if response.status_code == 200:
        ...
        st.error(f"Erro ao obter pull requests: {response.status_code}")
        return []

# Função para obter todas as iterações de um pull request
def obter_iteracoes_pull_request(organization, project, repo, pull_request_id, pat):
    url = f"https://dev.azure.com/{organization}/{project}/_apis/git/repositories/{repo}/pullRequests/{pull_request_id}/iterations?api-version=6.0"
    headers = {
        'Content-Type': 'application/json'
    }
    response = requests.get(url, headers=headers, auth=HTTPBasicAuth('', pat))
    if response.status_code == 200:
        ...
        st.error(f"Erro ao obter iterações do pull request: {response.status_code}")
        return []

# Função para obter as mudanças de uma iteração específica
def obter_mudancas_iteracao(organization, project, repo, pull_request_id, iteration_id, pat):
    url = f"https://dev.azure.com/{organization}/{project}/_apis/git/repositories/{repo}/pullRequests/{pull_request_id}/iterations/{iteration_id}/changes?api-version=6.0"
    headers = {
        'Content-Type': 'application/json'
    }
    response = requests.get(url, headers=headers, auth=HTTPBasicAuth('', pat))
    if response.status_code == 200:
        ...
        st.error(f"Erro ao obter as mudanças da iteração: {response.status_code}")
        return []

# Função para obter o conteúdo de um arquivo específico
def obter_conteudo_arquivo(organization, project, repo, path, commit_id, pat):
    url = f"https://dev.azure.com/{organization}/{project}/_apis/git/repositories/{repo}/items?path={path}&versionDescriptor.version={commit_id}&api-version=6.0"
    headers = {
        'Content-Type': 'application/json'
    }
    response = requests.get(url, headers=headers, auth=HTTPBasicAuth('', pat))
    if response.status_code == 200:
        ...
        st.error(f"Erro ao obter o conteúdo do arquivo: {response.status_code}")
        return ""

# Função para buscar e analisar pull requests do Azure DevOps
def analisar_pull_requests():
    pull_requests = obter_pull_requests(ORGANIZATION, PROJECT, REPO, PERSONAL_ACCESS_TOKEN)
    analisados = []
    if pull_requests:
        for pr in pull_requests:
            st.subheader(f"Pull Request ID: {pr['pullRequestId']}")
            st.write(f"Título: {pr['title']}")
            st.write(f"Descrição: {pr['description']}")
            st.write(f"Autor: {pr['createdBy']['displayName']}")

            iterations = obter_iteracoes_pull_request(ORGANIZATION, PROJECT, REPO, pr['pullRequestId'], PERSONAL_ACCESS_TOKEN)
            if iterations:
                last_iteration = iterations[-1]
                changes = obter_mudancas_iteracao(ORGANIZATION, PROJECT, REPO, pr['pullRequestId'], last_iteration['id'], PERSONAL_ACCESS_TOKEN)
                if changes:
                    for change in changes:
                        item = change['item']
                        if change['changeType'] == 'edit':
                            original_content = obter_conteudo_arquivo(ORGANIZATION, PROJECT, REPO, item['path'], last_iteration['sourceRefCommit']['commitId'], PERSONAL_ACCESS_TOKEN)
                            altered_content = obter_conteudo_arquivo(ORGANIZATION, PROJECT, REPO, item['path'], last_iteration['targetRefCommit']['commitId'], PERSONAL_ACCESS_TOKEN)
                            st.write(f"Arquivo: {item['path']}")
                            col1, col2 = st.columns(2)
                            with col1:
                                st.text_area("Original", original_content, height=200)
                            with col2:
                                st.text_area("Alterado", altered_content, height=200)

                            # Gerar resposta da IA para a análise do pull request
                            if st.button(f"Analisar {item['path']}"):
                                first_training_file = next(iter(training_files.values()))
                                message = f"Original: {original_content}\nAlterado: {altered_content}"
                                response = generate_response(message, st.session_state.llm_chain, first_training_file)
                    ...
                    st.write("Nenhuma mudança encontrada na última iteração.")
            else:
                st.write("Nenhuma iteração encontrada para este pull request.")
    else:
...
if menu in ["Python", "C++", "Java"]:
    ...
    if submenu == "Treinar Novo Arquivo":
        ...
        analisados = analisar_pull_requests()
        if analisados:
            st.subheader("Histórico de Análises de Pull Requests")
            for idx, analysis in enumerate(analisados):
                st.write(f"**Análise {idx + 1}:**")
                st.write(f"Arquivo: {analysis['arquivo']}")
                col1, col2 = st.columns(2)
                with col1:
                    st.text_area("Original", analysis['original'], height=200)
                with col2:
                    st.text_area("Alterado", analysis['alterado'], height=200)
                st.write(f"**Avaliação:** {analysis['avaliacao']}")
